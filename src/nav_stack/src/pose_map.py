...
#             'rear': CameraConfig('/back_cam/aruco/markers', 'back_cam_optical_frame'),
...
class ArUcoMapAnnotationNode(Node):
    def __init__(self):
        ...
        # Anomaly path
        self.pose_pattern = re.compile(r"(front|rear|left|right)__(-?\d+\.?\d*)_(-?\d+\.?\d*)_(-?\d+\.?\d*)")
        self.anomaly_marker_color = (255,192,203)
        ...
        # Core parameters
        self.target_frame = 'map'
        self.robot_frame = 'panther/base_link'
        self.starting_pose_delay = 3.0
        self.auto_save_on_shutdown = True
        self.map_save_directory = './saved_maps'
        self.pose_smoothing_weight = 0.3
    ...
    def annotate_map_image(self, map_image_path: str) -> str:
        ...
        try:
            ...
            #anomaly mark (blue)
            folder_path = '/home/farazn/ros2_ws/anomaly_images'
            folder = Path(folder_path)
            if not folder.exists() or not folder.is_dir():
                self.get_logger().error(f"Invalid folder path: {folder_path}")
        
            image_files = list(folder.glob("*.[pP][nN][gG]"))
            if not image_files:
                self.get_logger().warn(f"No images found in folder: {folder_path}")
            
        
            for img_path in image_files:
                match = self.pose_pattern.search(img_path.stem)
                if not match:
                    self.get_logger().warn(f"No pose info in filename: {img_path.name}")
                    continue
            
                cam, x_str, y_str, z_str = match.groups()
                try:
                    x, y, z = float(x_str), float(y_str), float(z_str)
                except Exception as e:
                    self.get_logger().warn(f"Failed to parse coordinates in {img_path.name}: {e}")
                    continue
            
                # Create a dummy PoseStamped for this extracted pose
                #pose = PoseStamped()
                ...
                anomaly_x,anomaly_y = self.world_to_map_coords(
                        x,y
                    )
                if anomaly_x is not None and anomaly_y is not None:
                        # Draw filled circle for marker
                        cv2.circle(img_color, (anomaly_x, anomaly_y), self.annotation_radius, 
                                  self.anomaly_marker_color, -1)  # -1 for filled circle
                        
                        #cv2.putText(img_color, "anomaly", (marker_x + 15, marker_y - 10),
                                #  cv2.FONT_HERSHEY_SIMPLEX, 0.5, self.text_color, self.text_thickness)
                        
                        self.get_logger().info(f"Annotated Anomaly marker at ({anomaly_x}, {anomaly_y})")    
